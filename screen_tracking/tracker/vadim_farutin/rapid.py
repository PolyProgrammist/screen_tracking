...
from screen_tracking.tracker.vadim_farutin.utils_vadim_farutin import project_points_int, rodrigues, \
    VadimFarutinTrackerAdapter
from screen_tracking.tracker.vadim_farutin.tracker_params import TrackerParams


class RapidTrackerVadimFarutin:
    ...
    def track(self, frame1_grayscale_mat, frame2_grayscale_mat,
        ...
        control_points, control_points_pair = self.control_points(
            self.object_points, RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER)
        control_points = [control_points[i * RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER:
                                         (i + 1) * RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER]
                          for i in range(RapidTrackerVadimFarutin.EDGE_NUMBER)]
        control_points_pair = [control_points_pair[i * RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER:
                                                   (i + 1) * RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER]
                               for i in range(RapidTrackerVadimFarutin.EDGE_NUMBER)]
        ...
        for i in range(RapidTrackerVadimFarutin.EDGE_NUMBER):
            ...
            error = RapidTrackerVadimFarutin.find_edge_error(found_points, corners)
        ...
        all_control_points = np.array([control_points[i // RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER]
    ...
    def search_edge(self, r, s, gradient_map, edge_index):
        ...
        for j in range(RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER):
            step_x, step_y = RapidTrackerVadimFarutin.get_search_direction(tana[j])
            ...
            found_points_idx.append(
                edge_index * RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER + j)
    ...
    def search_edge_from_point(self, edges, start, step):
        ...
        max_gradient, max_gradient_point = \
            self.search_edge_from_point_to_one_side(
                edges, start, step, RapidTrackerVadimFarutin.NUMBER_OF_STEPS,
                max_gradient, max_gradient_point)
        step = (-step[0], -step[1])
        max_gradient, max_gradient_point = \
            self.search_edge_from_point_to_one_side(
    ...
    def find_edge_error(found_points, corners):
        ...
        error /= RapidTrackerVadimFarutin.EDGE_CONTROL_POINTS_NUMBER
    ...
    def filter_edge_points(found_points, found_points_idx, corners):
        ...
        for point, j in zip(found_points, found_points_idx):
            d = np.linalg.norm(
                np.cross(corners[1] - corners[0], corners[0] - point)) \
                / np.linalg.norm(corners[1] - corners[0])
            if d <= RapidTrackerVadimFarutin.EDGE_POINT_FILTER_THRESHOLD:
...
class RapidTracker(VadimFarutinTrackerAdapter):
    ...
    def get_tracker_class(self):
        return RapidTrackerVadimFarutin